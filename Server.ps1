...
# decide `Write-Verbose` output
# - "SilentlyContinue",
# - "Continue", same effect as `-Verbose` argument
...
While ($True) {
	...
	While ($Socket_TCP_Client.Connected) {
        ...
        try{
            ...
            if($string){
                Write-Verbose "Stream data available."
            }
            else{
                Write-Verbose "Empty stream."
            }
        }
        catch{
            # Exception：
        ...
        if ($string) {
            ...
            if($Is_Path_Exist_Folder){
			    ...
			    $Stream_Writer.WriteLine("-1")
                $Stream_Writer.Flush()
            }
		}
        elseif($string -eq $Null -or (($Current_Time = (Get-Date -UFormat %s)) - $Previous_Communication_Time) -gt 3){ #等待数据超时，检查断线
            $bytes = 0
            try{ #heart-beat
                Write-Verbose "Heart-beat test."
                # see：[(16)Powershell中的转义字符 - yang-leo - 博客园](https://www.cnblogs.com/leoyang63/articles/12060596.html)
    ...
    if($string -eq "exit"){
        ...
        Write-Host("Client {0} disconnect." -f $Client_Address)
    }
}
